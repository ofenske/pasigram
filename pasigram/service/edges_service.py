...
def compute_unique_edges(edges_with_node_labels: pd.DataFrame) -> pd.DataFrame:
    ...
    edges_with_node_labels : pd.DataFrame
        Contains all edges with node labels as entries for the source and target columns of the input graph
...
def compute_frequent_edges(unique_edges: pd.DataFrame, min_support: int) -> pd.DataFrame:
    ...
    unique_edges : pd.DataFrame
        Contains all unique edges
    min_support : int
        The minimum support the edges have to meet
